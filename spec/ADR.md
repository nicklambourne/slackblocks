...
- **Limits are sourced three ways.** Java and C# generate them from `limits.json`, TypeScript imports it at runtime, and Python and Go hardcoded them. ADR 0001 said to reconsider generated constants once three maintained languages made scalar drift costly; there are now five.
...
3. **Every implementation takes its limit values from `limits.json`:** by direct import where the package can bundle the file (TypeScript), otherwise as checked-in generated constants with a test that fails on drift (Python, Go, Java, C#). This supersedes ADR 0001's allowance to hardcode them.
