...
3. **Every implementation takes its limit values from `limits.json`:** by direct import where the package can bundle the file (TypeScript), otherwise as checked-in generated constants that CI regenerates and diffs (Python, Go, Java, C#). This supersedes ADR 0001's allowance to hardcode them. All four languages that generate constants now validate against them; limits without a `limits.json` leaf, such as required-field minimums, remain implementation checks.
