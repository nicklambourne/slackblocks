...
def test_rich_text_channel_basic() -> None:
    assert fetch_sample(path="rich_text/rich_text_channel_basic.json") == repr(
        RichTextChannel(
...
def test_rich_text_emoji_basic() -> None:
    assert fetch_sample(path="rich_text/rich_text_emoji_basic.json") == repr(
        RichTextEmoji(
            name="wave",
        )
    )
...
def test_rich_text_link_basic() -> None:
    assert fetch_sample(path="rich_text/rich_text_link_basic.json") == repr(
        RichTextLink(
...
def test_rich_text_basic() -> None:
    assert fetch_sample(path="rich_text/rich_text_basic.json") == repr(
        RichText(
...
def test_rich_text_user_basic() -> None:
    assert fetch_sample(path="rich_text/rich_text_user_basic.json") == repr(
        RichTextUser(
            user_id="DR36TNNLA",
            bold=True,
            italic=False,
...
def test_rich_text_user_group_basic() -> None:
    assert fetch_sample(path="rich_text/rich_text_user_group_basic.json") == repr(
        RichTextUserGroup(
...
def test_rich_text_list_basic() -> None:
    assert fetch_sample(path="rich_text/rich_text_list_basic.json") == repr(
        RichTextList(
            elements=[
                RichTextSection(
                    elements=[
...
def test_rich_text_list_ordered() -> None:
    assert fetch_sample(path="rich_text/rich_text_list_ordered.json") == repr(
        RichTextList(
            elements=[
                RichTextSection(
                    elements=[
...
def test_rich_tex_code_block_basic() -> None:
    assert fetch_sample(path="rich_text/rich_text_code_block_basic.json") == repr(
        RichTextCodeBlock(
...
def test_rich_text_quote_basic() -> None:
    assert fetch_sample(path="rich_text/rich_text_quote_basic.json") == repr(
        RichTextQuote(elements=[RichText(text="Great and good are seldom the same man")], border=1),
    )


def test_rich_text_section() -> None:
    assert fetch_sample(path="rich_text/rich_text_section_basic.json") == repr(
        RichTextSection(
