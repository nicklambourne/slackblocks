...
func TestSharedInvalidFixtures(t *testing.T) {
	...
	if len(manifest.Cases) != 130 {
		t.Fatalf("expected 130 invalid cases, got %d", len(manifest.Cases))
	}
...
func validRows() [][]slackblocks.DataTableCell {
	return [][]slackblocks.DataTableCell{{rawText("Name")}, {rawText("Alice")}}
}
...
func invalidConstruction(id string) error {
	...
	case "select-too-many-option-groups":
		return buildError(slackblocks.NewStaticSelect().ActionID("a").OptionGroups(copies(101, func(int) *slackblocks.OptionGroupBuilder {
			return slackblocks.NewOptionGroup().Label("Group").Options(choice())
		})...))
	case "overflow-empty":
		return buildError(slackblocks.NewOverflow().ActionID("a").Set("options", []any{}))
	case "overflow-too-many-options":
		return buildError(slackblocks.NewOverflow().ActionID("a").Options(copies(6, func(int) *slackblocks.OptionBuilder { return choice() })...))
	...
	case "checkboxes-too-many-options":
		return buildError(slackblocks.NewCheckboxes().ActionID("a").Options(copies(11, func(int) *slackblocks.OptionBuilder { return choice() })...))
	...
	case "actions-too-many-elements":
		return buildError(slackblocks.NewActionsBlock().Elements(copies(26, func(int) slackblocks.Element { return slackblocks.NewButton().Text("A").ActionID("a") })...))
	case "context-too-many-elements":
		return buildError(slackblocks.NewContextBlock().Elements(copies(11, func(int) slackblocks.ContextElement { return slackblocks.NewMarkdown().Text("A") })...))
	...
	case "message-too-many-blocks":
		return buildError(slackblocks.NewMessage().Channel("C123").Blocks(copies(51, func(int) slackblocks.Block { return slackblocks.NewDividerBlock() })...))
	case "message-too-many-attachments":
		return buildError(slackblocks.NewMessage().Channel("C123").Attachments(copies(101, func(int) *slackblocks.AttachmentBuilder {
			return slackblocks.NewAttachment().Blocks(slackblocks.NewDividerBlock())
		})...))
	...
	case "context-invalid-element":
		return buildError( // The typed API rejects a divider here at compile time; Set checks the runtime validator.
			slackblocks.NewContextBlock().Set("elements", []any{slackblocks.NewDividerBlock()}))
	case "input-invalid-element":
		return buildError( // The typed API rejects a button here at compile time; Set checks the runtime validator.
			slackblocks.NewInputBlock().Label("Label").Set("element", slackblocks.NewButton().Text("A").ActionID("a")))
	...
	case "card-too-many-actions":
		return buildError(slackblocks.NewCardBlock().Actions(copies(4, func(int) *slackblocks.ButtonBuilder { return slackblocks.NewButton().Text("A").ActionID("a") })...))
	...
	case "carousel-too-many-cards":
		return buildError(slackblocks.NewCarouselBlock().Elements(copies(11, func(int) *slackblocks.CardBlockBuilder { return validCard() })...))
	...
	case "container-too-many-child-blocks":
		return buildError(slackblocks.NewContainerBlock().Title("Container").ChildBlocks(copies(11, func(int) slackblocks.Block { return slackblocks.NewDividerBlock() })...))
	case "context-actions-too-many-elements":
		return buildError(slackblocks.NewContextActionsBlock().Elements(copies(6, func(int) slackblocks.ContextActionsElement { return slackblocks.NewIconButton().Text("Delete") })...))
	...
	case "data-table-too-many-rows":
		return buildError(slackblocks.NewDataTableBlock().Rows(copies(202, func(int) []slackblocks.DataTableCell { return []slackblocks.DataTableCell{rawText("A")} })...).Caption("Names"))
	case "data-table-too-few-columns":
		return buildError(slackblocks.NewDataTableBlock().Rows([]slackblocks.DataTableCell{}, []slackblocks.DataTableCell{}).Caption("Empty"))
	case "data-table-too-many-columns":
		return buildError(slackblocks.NewDataTableBlock().Rows(copies(2, func(int) []slackblocks.DataTableCell {
			return copies(21, func(int) slackblocks.DataTableCell { return rawText("A") })
		})...).Caption("Wide"))
	...
	case "data-table-cell-text-empty":
		return buildError(slackblocks.NewDataTableBlock().Rows([]slackblocks.DataTableCell{rawText("Name")}, []slackblocks.DataTableCell{rawText("")}).Caption("Names"))
	case "data-table-content-too-long":
		return buildError(slackblocks.NewDataTableBlock().Rows([]slackblocks.DataTableCell{rawText("Name")}, []slackblocks.DataTableCell{rawText(repeated("x", 20000))}).Caption("Names"))
	...
	case "pie-chart-too-many-segments":
		return buildError(slackblocks.NewPieChart().Segments(copies(13, func(int) *slackblocks.ChartSegmentBuilder { return slackblocks.NewChartSegment().Label("S").Value(1) })...))
	...
	case "chart-too-many-series":
		return buildError(slackblocks.NewLineChart().Series(copies(13, func(int) *slackblocks.DataSeriesBuilder { return validSeries("S") })...).AxisConfig(validAxis()))
	...
	case "data-series-too-many-points":
		return buildError(slackblocks.NewDataSeries().Name("Series").Data(copies(21, func(index int) *slackblocks.DataPointBuilder {
			return slackblocks.NewDataPoint().Label("P").Value(float64(index))
		})...))
