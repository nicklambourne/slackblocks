...
func main() {
	...
	docs, shared := loadDocs(root)
	types := newTyping(shared)
	...
	output.WriteString(types.markerDeclarations(shared))
	for _, constructor := range constructors {
		...
		doc, documented := docs[constructor]
		if documented {
			output.WriteString(doc.typeComment)
		} else {
		...
		for _, methodName := range registry[constructor] {
			...
			field, typed := doc.fields[methodName]
			if !documented || !typed {
				panic(fmt.Sprintf("%s.%s is missing from %s", constructor, methodName, modelFile))
			}
			output.WriteString(types.typedMethods(typeName, field, doc.methods[methodName]))
		}
	}
