...
`slackblocks` builds Slack messages with the [Block Kit API](https://api.slack.com/block-kit) in Python, TypeScript, Go, and Java — without writing JSON by hand.
...
- **Typed, idiomatic APIs** for Python, TypeScript, Go, and Java.
...
- **A small runtime footprint.** Python has no runtime dependencies, TypeScript emits a self-contained ESM module, Go integrates with `slack-go/slack`, and Java implements the official Slack SDK block interfaces.
