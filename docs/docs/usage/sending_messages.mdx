...
import { section, webhookMessage } from "slackblocks";

const webhook = new IncomingWebhook(process.env.SLACK_WEBHOOK_URL!);
await webhook.send(webhookMessage({
  blocks: [section({ text: "Build complete :white_check_mark:" })],
}));
```
...
```ts
import { messageResponse, section } from "slackblocks";

const responseBody = messageResponse({
  blocks: [section({ text: "Got it! Working on that now..." })],
...
```ts
import { modal, section } from "slackblocks";

const view = modal({
  title: "Confirm action",
  blocks: [section({ text: "Are you sure?" })],
  submit: "Yes",
  close: "Cancel",
});
