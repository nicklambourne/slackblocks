...
Categories are identical across Python, TypeScript, Go, and Java: `length-exceeded`, `out-of-range`, `mutually-exclusive`, `type-mismatch`, `missing-required`, and `invalid-usage`.
...
- The newer Block Kit shapes (tables, cards, carousels, charts, plans) and the same JSON as the Python, TypeScript, and Java packages.
