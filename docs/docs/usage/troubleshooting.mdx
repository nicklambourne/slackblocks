...
Slack supports `mrkdwn` and `plain_text`. The `section()` factory turns a string into `mrkdwn` by default, but Slack requires headers, button labels, modal titles, input labels, and option labels to use `plain_text`. Markdown syntax in those fields is displayed literally.
...
```ts
import { header, message } from "slackblocks";

const payload = message({
  channel: "C0123456",
  text: "Build #482 passed",
  blocks: [header({ text: "Build #482 passed :white_check_mark:" })],
});
```
...
```ts
import { actions, button } from "slackblocks";
...
| Factory field                       | Limit |
|-------------------------------------|-------|
| `section().text`                    | 3,000 characters |
| `section().fields[i]`               | 2,000 characters |
...
- Modals and home tabs: **100 blocks**.
- `context()`: up to **10 elements**.
- `actions()`: up to **25 elements**.
- `section().fields`: up to **10 items**.
- `table()`: up to **100 rows × 20 columns**.
...
## Why doesn't markdown work inside `header()`?

Slack only accepts `plain_text` in header blocks. Use `section()` followed by `divider()` when you need styled heading text.
...
```ts
import { blockKitBuilderUrl, message, section } from "slackblocks";
