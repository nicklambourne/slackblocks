...
```ts
import { context, divider, header, message, mrkdwn, section } from "slackblocks";
...
function buildStatusMessage(input: {
  ...
  return message({
    ...
    blocks: [
      header({ text: `${icon} ${input.project} build ${status}` }),
      section({
        fields: [
          `*Branch*\n\`${input.branch}\``,
      ...
      divider(),
      context({ elements: [mrkdwn(`:calendar: Triggered by push to \`${input.branch}\``)] }),
    ],
  });
}
```
