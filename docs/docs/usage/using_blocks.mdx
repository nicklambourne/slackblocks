...
TableBlock block = TableBlock.builder()
    .blockId("fake_block_id")
    .columnSettings(
        ColumnSettings.builder().align("right").isWrapped(true).build(),
        ColumnSettings.builder().align("left").build())
    .rows(
        List.of(
            RichTextBlock.builder().elements(RichTextSection.builder()
                .elements(RichTextText.builder().text("Header 1")
                    .wireField("style", Map.of("bold", true)).build()).build()).build(),
            RichTextBlock.builder().elements(RichTextSection.builder()
...
CardBlock block = CardBlock.builder()
    .title("Build complete")
    .body("Version 2.3.0 is ready to deploy.")
...
PlanBlock block = PlanBlock.builder()
    .title("Release plan")
    .tasks(
        TaskCardBlock.builder().taskId("test").title("Run the test suite").status("complete").build(),
        TaskCardBlock.builder().taskId("deploy").title("Deploy the release").status("pending").build())
    .blockId("fake_block_id")
    .build();
```
  ...
  .blocks(
    Accordion().sections(
      AccordionSection()
        .title("Summary")
        .expanded(true)
        .blocks(SectionBlock().text("Version 2.3.0 is ready.")),
      AccordionSection()
        .title("Checks")
...
blocks, err := slackblocks.NewAccordion().Sections(
	slackblocks.NewAccordionSection().
		Title("Summary").
		Expanded(true).
		Blocks(slackblocks.NewSectionBlock().Text("Version 2.3.0 is ready.")),
	slackblocks.NewAccordionSection().
		Title("Checks").
...
List<Block> blocks = Accordion.builder()
    .sections(
        AccordionSection.builder("Summary")
            .expanded(true)
            .blocks(SectionBlock.builder().markdownText("Version 2.3.0 is ready.").build())
            .build(),
