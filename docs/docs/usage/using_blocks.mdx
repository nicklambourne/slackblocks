...
For constructors, typed properties, and validation errors, see the [C# API reference](../reference/csharp). Interactive controls such as buttons, menus, and date pickers are grouped under [Elements](../reference/csharp/elements).
...
Accordion sections use Slack-native collapsible containers and need no application-side expansion handler. See [Components](../reference/csharp/components) for the complete API.
